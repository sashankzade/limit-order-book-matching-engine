...
static void BM_Get_Order(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        ...
        book.AddOrder(bid);
        auto start = std::chrono::high_resolution_clock::now();
        (void) book.ContainsOrder(bid.OrderId());
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed_seconds =
                std::chrono::duration_cast<std::chrono::duration<double> >(
                    end - start);
...
static void BM_Run_Simulation(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        long bestBid = book.GetBestBidPrice().value_or(lastBid);
        long bestAsk = book.GetBestAskPrice().value_or(lastAsk);
        long MIN_DEVIANCE = 1 + rand() % (bestAsk - bestBid);
        double midPrice = static_cast<double>(bestAsk + bestBid) / 2;
        // 2 limit orders
