...
static OrderBook<mdfeed::NullMarketDataPublisher> createOrderBook()
{
    ...
    auto publisher = mdfeed::NullMarketDataPublisher();
...
TEST(OrderBookTests, CanAddBids)
{
    ...
    Security apl("apple", "AAPL", SECURITY_ID);
    auto book = createOrderBook();
    Order order(OrderCore(USERNAME, SECURITY_ID), 50, 20, true);
    book.AddOrder(order);
    EXPECT_EQ(book.Count(), 1);
    EXPECT_TRUE(book.ContainsOrder(order.OrderId()));
    ...
    EXPECT_EQ(bids.begin()->CurrentOrder().OrderId(), order.OrderId());
}
...
TEST(OrderBookTests, CanAddAsks)
{
    ...
    Security apl("apple", "AAPL", SECURITY_ID);
    auto book = createOrderBook();
    Order order(OrderCore(USERNAME, SECURITY_ID), 45, 15, false);
    book.AddOrder(order);
    EXPECT_EQ(book.Count(), 1);
    EXPECT_TRUE(book.ContainsOrder(order.OrderId()));
...
TEST(OrderBookTests, CanCancelBids)
{
    ...
    Security apl("apple", "AAPL", SECURITY_ID);
    auto book = createOrderBook();
    const Order order(OrderCore(USERNAME, SECURITY_ID), 50, 20, true);
    book.AddOrder(order);
...
TEST(OrderBookTests, CanModifyBids)
{
    ...
    book.AmendOrder(order.OrderId(), modifiedOrder);
    EXPECT_EQ(book.Count(), 1);
    std::list<OrderBookEntry> bids = book.GetBidOrders();
    EXPECT_EQ(bids.begin()->GetLimit()->Price(), 50);
    EXPECT_EQ(bids.begin()->GetLimit()->IsEmpty(), false);
