...
PYBIND11_MODULE(_orderbook, m)
{
    ...
    py::class_<Security>(m, "Security")
            .def(py::init([](const std::string_view name, const std::string_view ticker, int security_id) {
    ...
    py::class_<OrderCore>(m, "OrderCore")
            .def(py::init<const std::string_view, int>(),
                "Create OrderCore with auto-generated ID",
                py::arg("username"), py::arg("security_id"))
            .def(py::init<long, const std::string_view, int>(),
    ...
    m.def("create_order", [](const std::string_view username, int security_id,
