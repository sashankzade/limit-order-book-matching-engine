...
struct MessageHeader {
    ...
    [[nodiscard]] std::string toDebugString() const
    {
        ...
        oss << "Header[seq=" << sequence_number << ", len=" << message_length << ", type=" << message_type << ", ts=" <<
                timestamp_ns << ", inst=" << instrument_id << "]";
        return oss.str();
    }
};
...
struct NewOrderMessage {
    ...
    [[nodiscard]] std::string toDebugString() const
    {
        ...
        oss << "NEW_ORDER: " << header.toDebugString()
                ...
                << static_cast<double>(price) / 100 << ", qty=" << quantity
...
struct ModifyOrderMessage {
    ...
    [[nodiscard]] std::string toDebugString() const
    {
        ...
        oss << "MODIFY_ORDER: " << header.toDebugString()
                ...
                << static_cast<double>(new_price) / 100 << ", new_qty=" << new_quantity
                << ", symbol=" << std::string(symbol, 8);
        return oss.str();
    }
};
...
struct ExecutionReportMessage {
    ...
    [[nodiscard]] std::string toDebugString() const
    {
        ...
        oss << "EXECUTION_REPORT: " << header.toDebugString()
                ...
                << std::setprecision(2) << static_cast<double>(execution_price) / 100
...
inline std::string format_timestamp(const uint64_t timestamp_ns)
{
    ...
    const std::tm* tm_ptr = std::localtime(&time_seconds);
    std::ostringstream oss;
    oss << std::put_time(tm_ptr, "%H:%M:%S") << '.' << std::setw(3) << std::setfill('0') << ms;
    return oss.str();
}

inline std::string_view message_type_to_string(const MessageType type)
{
    switch (type) {
