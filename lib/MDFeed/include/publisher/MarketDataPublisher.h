...
public:
    explicit MarketDataPublisher(PublisherConfig config = PublisherConfig{});
    ~MarketDataPublisher();
    ...
    [[nodiscard]] std::shared_ptr<MDRingBuffer> get_ring_buffer() const;

private:
    PublisherConfig config_;
    std::shared_ptr<MDRingBuffer> ring_buffer_;
...
public:
    static bool publish_price_level_update(uint32_t, uint64_t, uint64_t, Side, UpdateAction) { return true; }
    static bool publish_price_level_delete(uint32_t, uint64_t, Side) { return true; }
    static bool publish_trade(uint32_t, uint64_t, uint64_t, uint64_t, Side) { return true; }
    static bool publish_book_clear(uint32_t, uint32_t) { return true; }
};
}
