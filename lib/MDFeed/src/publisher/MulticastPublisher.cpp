...
public:
    ...
    bool create_and_bind(const std::string_view multicast_ip, uint16_t port, const std::string_view interface_ip)
    {
        ...
        if (inet_aton(multicast_ip.data(), &multicast_addr_.sin_addr) == 0) {
            close();
            return false;
        }

        in_addr local_interface{};
        if (inet_aton(interface_ip.data(), &local_interface) == 0) {
            close();
            return false;
        }
        ...
        const int send_buffer_size = 1024 * 1024; // 1MB send buffer
        setsockopt(socket_fd_, SOL_SOCKET, SO_SNDBUF, &send_buffer_size, sizeof(send_buffer_size));
        return true;
    }

    bool send_data(const void* data, const std::size_t length) const
    {
        if (socket_fd_ < 0) {
            return false;
        }
...
MulticastPublisher::MulticastPublisher(const std::string_view multicast_ip, const uint16_t port,
    const std::string_view interface_ip)
    : socket_(std::make_unique<MulticastSocket>()), initialized_(false)
{
...
bool MulticastPublisher::initialize(const std::string_view multicast_ip, const uint16_t port,
    const std::string_view interface_ip)
{
    if (initialized_) {
        close();
...
bool MulticastPublisher::send(const void* data, const std::size_t length) const
{
