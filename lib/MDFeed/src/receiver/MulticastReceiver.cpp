...
public:
    ...
    bool create_and_join(const std::string_view multicast_ip, uint16_t port, const std::string_view interface_ip,
        const size_t buffer_size)
    {
        ...
        setsockopt(socket_fd_, SOL_SOCKET, SO_RCVBUF, &recv_buffer_size, sizeof(recv_buffer_size));
        ...
        ip_mreq mreq{};
        if (inet_aton(multicast_ip.data(), &mreq.imr_multiaddr) == 0) {
            close();
            return false;
        }

        if (inet_aton(interface_ip.data(), &mreq.imr_interface) == 0) {
            close();
            return false;
        }
...
MulticastReceiver::MulticastReceiver()
    : socket_(std::make_unique<MulticastSocket>()), running_(false), last_sequence_number_(0)
{
...
void MulticastReceiver::receiver_loop()
{
    ...
    log_message(
        "MulticastReceiver started - listening on " + config_.multicast_ip + ":" + std::to_string(
            config_.multicast_port));

    while (running_.load()) {
        if (ssize_t bytes_received = socket_->receive_data(buffer.get(), BUFFER_SIZE); bytes_received > 0) {
...
void MulticastReceiver::process_message(const void* data, const size_t length)
{
    ...
    if (header->message_length != length) {
        stats_.invalid_messages++;
        log_message(
            "Message length mismatch: header says " + std::to_string(header->message_length) + ", received " +
            std::to_string(length));
        return;
    }
    if (config_.validate_sequence_numbers && last_sequence_number_ > 0) {
        if (header->sequence_number != last_sequence_number_ + 1) {
            const uint64_t gap = header->sequence_number - last_sequence_number_ - 1;
...
std::string MulticastReceiver::format_message_debug(const MessageHeader& header, const void* data)
{
    ...
    case MessageType::HEARTBEAT: {
        const auto* msg = static_cast<const HeartbeatMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::PRICE_LEVEL_UPDATE: {
        const auto* msg = static_cast<const PriceLevelUpdateMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::PRICE_LEVEL_DELETE: {
        const auto* msg = static_cast<const PriceLevelDeleteMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::TRADE: {
        const auto* msg = static_cast<const TradeMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::SNAPSHOT_BEGIN: {
        const auto* msg = static_cast<const SnapshotBeginMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::SNAPSHOT_ENTRY: {
        const auto* msg = static_cast<const SnapshotEntryMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::SNAPSHOT_END: {
        const auto* msg = static_cast<const SnapshotEndMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    case MessageType::BOOK_CLEAR: {
        const auto* msg = static_cast<const BookClearMessage*>(data);
        oss << msg->toDebugString();
        break;
    }
    default: {
...
void MulticastReceiver::log_message(const std::string_view message) const
{
...
void MulticastReceiver::print_stats()
{
    ...
    const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - stats_.start_time);
