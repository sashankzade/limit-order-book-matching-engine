...
private:
    std::unordered_map<uint32_t, std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> > > order_books_;
    std::unordered_map<std::string, uint32_t> symbol_to_id_;
    std::unordered_map<uint32_t, std::string> id_to_symbol_;
    std::unordered_map<uint32_t, std::unique_ptr<mdfeed::MDAdapter<mdfeed::MarketDataPublisher> > > adapters_;
...
public:
    ...
    std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> > get_order_book(uint32_t symbol_id);

    std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> >
