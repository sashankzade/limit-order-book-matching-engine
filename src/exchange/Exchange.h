...
private:
    ...
    std::unordered_map<long, std::pair<int, uint64_t> > exchange_to_client_id_; // {fd, client_id}
...
public:
    Exchange(Mode mode, mdfeed::PublisherConfig md_config, uint16_t oe_port = 8080);
...
private:
    ...
    void simulate_trading_activity(const std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> >& order_book,
        uint32_t symbol_id, std::string_view symbol);
    void place_simulation_market_order(const std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> >& order_book,
        std::string_view symbol);
    void add_simulation_limit_order(const std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> >& order_book,
        uint32_t symbol_id, std::string_view symbol);
    ...
    static void send_order_ack(int client_fd, uint64_t client_order_id, long exchange_order_id,
        std::string_view symbol);
    static void send_order_reject(int client_fd, uint64_t client_order_id, uint32_t reason, std::string_view text);
    static void send_execution_report(int client_fd, uint64_t client_order_id, long exchange_order_id, uint64_t price,
        uint64_t executed_qty, uint64_t remaining_qty, orderentry::Side side);
