...
std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> > SymbolManager::get_order_book(uint32_t symbol_id)
{
    const auto it = order_books_.find(symbol_id);
    return it != order_books_.end() ? it->second : nullptr;
}

std::shared_ptr<OrderBook<mdfeed::MarketDataPublisher> > SymbolManager::get_order_book(const std::string& symbol)
{
...
std::optional<uint32_t> SymbolManager::get_symbol_id(const std::string& symbol) const
{
...
std::optional<std::string> SymbolManager::get_symbol_name(uint32_t symbol_id) const
{
...
void SymbolManager::initialize_symbols()
{
    ...
    for (const auto& [id, ticker, name] : symbols) {
        ...
        securities_[symbol_id] = std::make_unique<Security>(name, ticker, symbol_id);
        adapters_[symbol_id] = std::make_unique<mdfeed::MDAdapter<mdfeed::MarketDataPublisher> >(symbol_id,
            md_publisher_);
        order_books_[symbol_id] = std::make_unique<OrderBook<mdfeed::MarketDataPublisher> >(*securities_[symbol_id],
            *adapters_[symbol_id]);
