...
void print_usage(const char* program_name)
{
    ...
    std::cout << "  --server <address>    Server address (default: 127.0.0.1)" << std::endl;
...
void interactive_mode(orderentry::OrderEntryClient& client)
{
    ...
    std::cout << "  new <symbol> <side> <price> <quantity>  - Send new order (side: buy/sell)" << std::endl;
    std::cout << "  cancel <order_id> <symbol>              - Cancel order" << std::endl;
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (command == "new") {
            ...
            if (!(iss >> symbol >> side_str >> price >> quantity)) {
                std::cout << "Usage: new <symbol> <side> <price> <quantity>" << std::endl;
                std::cout << "> ";
                continue;
            }
            ...
            auto price_ticks = static_cast<uint64_t>(price * 100); // Convert to pence

            if (client.send_new_order(next_order_id, symbol, side, price_ticks, quantity)) {
            ...
            std::cout << "Commands:" << std::endl;
            std::cout << "  new <symbol> <side> <price> <quantity>" << std::endl;
